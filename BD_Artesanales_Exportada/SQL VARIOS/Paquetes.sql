...
-- Inventario
CREATE OR REPLACE EDITIONABLE PACKAGE "PROYECTO_DB"."PKG_INVENTARIO" AS
...
END pkg_inventario;
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PROYECTO_DB"."PKG_INVENTARIO" AS
    ...
    BEGIN
        OPEN v_Cursor FOR
        SELECT * FROM Inventario WHERE ID_Producto = p_ID_Producto;
        RETURN v_Cursor;
    END;
...
-- Informe Ventas
CREATE OR REPLACE EDITIONABLE PACKAGE "PROYECTO_DB"."PKG_INFORMES_VENTAS" AS
    FUNCTION Obtener_Ventas_Por_Cliente(
        p_ID_Cliente IN INT
    ) RETURN SYS_REFCURSOR;

    FUNCTION Obtener_Ventas_Por_Producto(
        p_ID_Producto IN INT
    ) RETURN SYS_REFCURSOR;

    FUNCTION Obtener_Ventas_Mensuales(
        p_Mes IN INT,
        p_Anio IN INT
    ) RETURN SYS_REFCURSOR;
END pkg_informes_ventas;
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PROYECTO_DB"."PKG_INFORMES_VENTAS" AS
    ...
    BEGIN
        OPEN v_Cursor FOR
        SELECT v.ID_Venta, v.Fecha_Venta, dv.Cantidad, dv.Precio_Unitario
    ...
    BEGIN
        OPEN v_Cursor FOR
        SELECT EXTRACT(DAY FROM Fecha_Venta) AS Dia, SUM(Total) AS Total_Ventas
        FROM Ventas
        WHERE EXTRACT(MONTH FROM Fecha_Venta) = p_Mes
        AND EXTRACT(YEAR FROM Fecha_Venta) = p_Anio
        GROUP BY EXTRACT(DAY FROM Fecha_Venta)
        ORDER BY Dia;
        RETURN v_Cursor;
    END;
